  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 text-center">
        <div className="mx-auto mb-3 size-14 rounded-full grid place-items-center bg-green-100 text-green-600 text-2xl">✓</div>
        <h3 className="text-xl font-semibold text-gray-900">Report Submitted</h3>
        <p className="mt-2 text-sm text-gray-600">Your case has been recorded successfully.</p>
        <p className="mt-1 text-sm font-medium text-gray-800">Case ID: <span className="font-bold">{caseId}</span></p>
        <div className="mt-6">
...
export default function CitizenPortal() {
    ...
    switch (activePage) {
      ...
      case 'my-reports':
        return (
          <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-6">
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white">My Reports</h1>
              <p className="text-slate-600 dark:text-slate-400 mt-1">Track your submitted cases and their current status.</p>
            </motion.div>
            <MyReports />
          </div>
        );
  ...
  return (
    <CitizenLayout activePage={activePage} setActivePage={setActivePage}>
      {renderContent()}
      <ConfirmationModal open={showConfirm} caseId={caseId} onClose={() => setShowConfirm(false)} />
    </CitizenLayout>
  );
}

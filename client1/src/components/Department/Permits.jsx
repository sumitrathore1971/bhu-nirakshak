...
import { FileCheck, Check, X, AlertTriangle, Eye, ChevronLeft, ChevronRight } from "lucide-react";
...
function StatusPill({ status }) {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${map[status] || "bg-gray-100 text-gray-800"}`}>
      {status}
    </span>
  );
}
...
export default function Permits() {
  ...
  return (
    <div className="p-6 space-y-4 bg-gray-50 dark:bg-neutral-950 min-h-[calc(100vh-72px)]">
      ...
      {flash?.message && (
        <div className={`px-4 py-2 rounded border text-sm ${flash.type === 'success' ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
          {flash.message}
        </div>
      )}
      <div className="flex items-center gap-2">
        {(["All","Pending","Approved","Suspicious","Rejected"]).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded border text-sm ${filter===f?"bg-black text-white":"bg-white"}`}>
            {f}
          </button>
        ))}
      </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((p, idx) => (
                  <tr key={p._id} className={idx % 2 === 0 ? "bg-background" : "bg-accent/30"}>
                    <td className="px-4 py-3 font-medium">{p.applicationId}</td>
                    <td className="px-4 py-3">{p.applicant?.name || "-"}</td>
                    <td className="px-4 py-3"><StatusPill status={p.urbanVerification?.status || "Pending"} /></td>
                    ...
                    <td className="px-4 py-3 text-right space-x-2">
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            setFlash({ message: `Application ${p.applicationId} forwarded to Revenue for ownership verification.`, type: 'success' });
                            setTimeout(() => setFlash(null), 3000);
                            await load();
                          } catch (e) {
                            setFlash({ message: e?.message || 'Failed to forward to Revenue', type: 'error' });
                      ...
                      <Button variant="outline" size="sm" className="gap-1" onClick={() => openViewer(p._id)}><Eye className="h-4 w-4" />View Plan</Button>
                      <Button variant="secondary" size="sm" className="gap-1" onClick={() => act(p._id, "Approved")}><Check className="h-4 w-4" />Approve</Button>
                      <Button variant="destructive" size="sm" className="gap-1" onClick={() => act(p._id, "Rejected")}><X className="h-4 w-4" />Reject</Button>
                      <Button variant="outline" size="sm" className="gap-1" onClick={() => flag(p._id)}><AlertTriangle className="h-4 w-4" />Flag</Button>
                    </td>
                  </tr>
                ))}
              </tbody>

...
export default function Dashboard() {
  ...
  const addCaseToDashboard = (reportData) => {
    ...
    console.log("📊 Adding new case to dashboard:", newCase);
    setCases((prev) => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedCases) {
        ...
        console.log(
          "📊 Loaded cases from localStorage in Dashboard:",
          parsedCases
        );
  ...
  useEffect(() => {
    ...
    const handleAssignToEnforcement = (data) => {
      console.log("📊 Received assignToEnforcement event in Dashboard:", data);
      if (data && data.report) {

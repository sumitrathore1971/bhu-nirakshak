...
export default function MyReports() {
  ...
  async function fetchReports() {
    try {
      ...
      const response = await reportService.getMyReports(currentPage, 10, statusFilter);
            ...
            {reports.length === 0 ? (
              <tr>
                <td colSpan="6" className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                  {statusFilter ? `No reports found with status "${statusFilter}"` : 'No reports found'}
                </td>
              </tr>
            ) : (
              reports.map((report) => (
                <tr key={report._id} className="hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors">
                  <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{report.reportId}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white max-w-xs truncate">{report.title}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{formatDate(report.createdAt)}</td>
                  <td className="px-6 py-4 text-sm text-gray-900 dark:text-white flex items-center gap-2">
                    <MapPin size={16} className="text-gray-400" />
                    {report.location.area || report.formattedAddress || 'Location'}
                  </td>
          ...
          >
            ...
            >
              <div className="p-6 border-b border-gray-200 dark:border-neutral-800 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-heading font-semibold text-gray-900 dark:text-white">{selected.reportId}</h3>
                  <p className="text-gray-600 dark:text-gray-400">{selected.title}</p>
                </div>
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{selected.category}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{formatDate(selected.dateOfObservation)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Submitted</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{formatDate(selected.createdAt)}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 dark:text-white flex items-center gap-2">
                    <MapPin size={16} className="text-gray-400" />
                    {selected.location.area || selected.formattedAddress || 'Location coordinates'}
                  </p>
                </div>
                  ...
                  <div className="flex items-center">
                    {stages.map((stage, idx) => (
                      <div key={stage} className="flex items-center">
                        <div className={`w-8 h-8 rounded-full grid place-items-center text-xs font-semibold ${
                          idx <= selected.stage ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'
                        }`}>
                          {idx + 1}
                        </div>
                        {idx < stages.length - 1 && (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {selected.media.map((media, i) => (
                        <div key={i} className="aspect-square bg-gray-200 dark:bg-neutral-800 rounded-lg flex items-center justify-center">
                          <span className="text-xs text-gray-500">{media.originalName}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selected.notes.map((note, i) => (
                        <div key={i} className="bg-gray-50 dark:bg-neutral-800 p-3 rounded-lg">
